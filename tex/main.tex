...
The logical Resnet-FPN backbone architecture is described as following: the input image is first feeded into the Resnet to extract feature maps at different levels, then these feature maps are passed through the FPN to generate a set of multi-scale feature maps, as shown in Figure~\ref{fig:fpn_architecture}. The anchor boxes in the RPN are then applied to these multi-scale feature maps in such a way that each level of the feature pyramid is responsible for detecting objects of specific scales, in particular, smaller objects are detected using higher-resolution feature maps, while larger objects are detected using lower-resolution feature maps, as shown in Figure~\ref{fig:resnet_fpn_combining}. 
